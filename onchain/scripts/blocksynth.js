...
async function main() {
  ...
  let res = await bs.createCoinbaseTx(genTx0, extraNonce1, extraNonce2, genTx1);
  console.log("Coinbase tx: " + res);
  let merkleRoot = await bs.coinbaseHash(res);
  console.log("Merkle root:", merkleRoot);
  let blockHeader = await bs.createBlockHeader(nonce, bits, nTime, merkleRoot,
    previousBlockHash, version);
  console.log("Block header:", blockHeader);
  let blockHash = await bs.blockHash(blockHeader);
